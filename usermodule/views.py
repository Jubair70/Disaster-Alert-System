...
from usermodule.models import UserModuleProfile, UserPasswordHistory, UserFailedLogin,Organizations,OrganizationDataAccess
...
def index(request):
    ...
    if current_user.is_superuser:
        query = """ SELECT auth_user.id, auth_user.first_name || ' ' || auth_user.last_name full_name, auth_user.username,(SELECT organization FROM usermodule_organizations WHERE id = usermodule_usermoduleprofile.organisation_name_id) organisation_name, coalesce((SELECT "role" FROM usermodule_organizationrole WHERE id = (select role_id from usermodule_userrolemap where user_id = usermodule_usermoduleprofile.id limit 1) LIMIT 1),'') role_name FROM auth_user, usermodule_usermoduleprofile WHERE auth_user.id = usermodule_usermoduleprofile.user_id """
        admin = True
    elif admin:
        org_id_list = get_organization_by_user(request.user)
        query = """ SELECT auth_user.id, auth_user.first_name || ' ' || auth_user.last_name full_name,  auth_user.username,(SELECT organization FROM usermodule_organizations WHERE id = usermodule_usermoduleprofile.organisation_name_id) organisation_name, coalesce((SELECT "role" FROM usermodule_organizationrole WHERE id = (select role_id from usermodule_userrolemap where user_id = usermodule_usermoduleprofile.id limit 1) LIMIT 1),'') role_name FROM auth_user, usermodule_usermoduleprofile WHERE auth_user.id = usermodule_usermoduleprofile.user_id and organisation_name_id =any('"""+str(org_id_list).replace('[','{').replace(']','}') +"""')"""
        admin = True
    else:
        query = """ SELECT auth_user.id, auth_user.first_name || ' ' || auth_user.last_name full_name, auth_user.username,(SELECT organization FROM usermodule_organizations WHERE id = usermodule_usermoduleprofile.organisation_name_id) organisation_name, coalesce((SELECT "role" FROM usermodule_organizationrole WHERE id = (select role_id from usermodule_userrolemap where user_id = usermodule_usermoduleprofile.id limit 1) LIMIT 1),'') role_name FROM auth_user, usermodule_usermoduleprofile WHERE auth_user.id = usermodule_usermoduleprofile.user_id and auth_user.id = """ + str(current_user.id)
        admin = False
    users = __db_fetch_values_dict(query)
    return render(request,'usermodule/index.html',{'users': users,'admin': admin,})
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            if 'image-file' in request.FILES:
                ...
                img_file_path = "/static/media/uploaded_files/" + myfile.name
...
def edit_profile(request,user_id):
    ...
    ,empty_label="Select a Organization")
            ...
            qry = "select *,user_img ursr from usermodule_usermoduleprofile where id = " + str(profile.pk)
            df = pandas.read_sql(qry, connection)
...
def change_password(request):
    ...
    if request.method == 'POST':
        ...
        if change_password_form.is_valid() and current_user is not None:
            ...
            return render(request,'usermodule/change_password.html',
    ...
    # This scenario would most likely be a HTTP GET.
        ...
        if request.GET.get('next'):
           ...
           #Presently USed Redirect URL
            redirect_url = '/modules/dashboard/'
