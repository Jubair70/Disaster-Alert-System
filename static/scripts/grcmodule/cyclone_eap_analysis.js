function init_union(statesData, union_dataobj, first_color_hex, last_color_hex, highest_value, lowest_value, location_prop, location_name, range_num, map_div_id, map_height, info_div,current_date,rank_dataobj,impact_dataobj){
    ...
    if(info_div == "info_div") stru = '<h5>Wind Speed Map on ' + current_date + '</h5>'
    ...
    var div = L.DomUtil.create('div', 'info legend'),
            ...
            if(map_div_id=='mapid')
            labels.push('<div class="mb-2 mr-2 badge badge-secondary">wind speed ( km)</div>');
...
$( "#flood-data-btn" ).click(function() {
     ...
     $.ajax({
                    url: '/modules/cyclone_wind_speed_data/',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                    },
                    ...
                    var columns = [];
                    var col_val = ["div_name", "dist_name", "upz_name", "union_name","union_code", "zone_label", "wind_speed"]
     ...
     $.ajax({
                    url: '/modules/cyclone_impact_data/',
                    type: 'POST',
                    dataType: 'json',
                    success: function (data_list) {
                    var all_data = data_list['df']
                    ...
                    var impact_dataobj = data_list['df_impact'];
                     var current_date = data_list['curr_date'];
                    var columns = [];
                    var col_val = ["div_name", "dist_name", "upz_name", "union_name","union_code", "zone_label",  "wind_speed", "impact"]
                    col_val.forEach(myFunction);
     ...
     function potential_data(){
                    $.ajax({
                    url: '/modules/cyclone_potential_data/',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                    },
                    ...
                    var current_date = data_list['curr_date'];
                    var columns = [];
                    var col_val = ["div_name", "dist_name", "upz_name", "union_name","union_code", "zone_label",  "wind_speed", "impact","vulnaribity_index","potential_damage","potential_damage_rank"]
                    col_val.forEach(myFunction);
                    function myFunction(item) {
                        var col_obj = {};
                        col_obj["data"] = item;
